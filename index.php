...
if (isset($_REQUEST['ENT_action'])) {
	...
	case 'INSCRIRE' :
	// --------------------------------------------------------------------------------
	// On suppose que le compte a dÈj‡ ÈtÈ provisionnÈ.
	// --------------------------------------------------------------------------------
		assert('$blogname != ""', "Le paramËtre \$blogname doit Ítre renseignÈ.");
		assert('$blogtype != ""', "Le paramËtre \$blogtype doit Ítre renseignÈ.");
		assert('$uid != ""', "Le paramËtre \$uid doit Ítre renseignÈ.");
		...
		// echo "<pre>
		// 	- VÈrifier que l'uid_admin ‡ bien un role superadmin ou  admin sur le blog.
		// 	- Appel ‡ l'annuaire pour vÈrifier les droits de s'inscrire 
		...
		// Ici on est ok pour les paramËtres
