...
// Hook pour la dŽsinscription d'un blog.
...
=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=*/
// selectbox pour rŽduire la liste par auteur
...
/*-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
	...
	s Ž c u r i s a t i o n   d e   l a   p l a t e f o r m e 
...
if (isset($_REQUEST['ENT_action'])) {
	...
	$mustDieAfterAction = false;  // UtilisŽ pour les actions qui ne nŽcessitent pas d'affichage aprs s'tre dŽroulŽes.
