	...
	case 'LISTE_ARCHIVAGE' : 
		...
		foreach($liste as $k => $blog) {
			if (!in_array($blog['siteurl'], $liste_a_conserver) && $blog['siteurl'] != "") {
				...
				if ($blog['archived'] == 0) {
					$html .= "<td><a href='?ENT_action=$ENT_action&action2=archiveblog&id=".$blog['blog_id']."&key=$clÈ_de_merde'>Archiver</a></td>";				
				} else {
					$html .= "<td>ArchivÈ !&nbsp;&nbsp;&nbsp;<a href='?ENT_action=$ENT_action&action2=unarchiveblog&id=".$blog['blog_id']."&key=$clÈ_de_merde'><span class='lilipute'>DÈsarchiver</span></a></td>";				
				}
				$html .= "</tr>\n";
			}
