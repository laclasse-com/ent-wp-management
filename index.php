...
if (isset($_REQUEST['ENT_action'])) {
	...
	//  --------------------------------------------------------------------------------
	//
	// API pour rÈcupÈrer le current user depuis l'annuaire v3
	//
	// ---------------------------------------------------------------------------------
	case 'CURRENT_USER' :
	...
	//  --------------------------------------------------------------------------------
	//
	// Front office de prÈsentation des blogs.
	//
	// ---------------------------------------------------------------------------------
	case 'FRONT' :
		if (phpCAS::isAuthenticated()) {
		...
		foreach($liste as $k => $blog) {
			if (!in_array("http://".$blog['domain']."/", $liste_a_conserver) && $blog['domain'] != "") {
				...
				if ($blog['archived'] == 0) {
					...
					$html .= "<td>ArchivÈ !&nbsp;&nbsp;&nbsp;<a href='?ENT_action=$ENT_action&action2=unarchiveblog&id=".$blog['blog_id']."#".($k+1)."'><span class='lilipute'>DÈsarchiver</span></a></td>";				
				}
				$html .= "</tr>\n";
			}
	...
	//  --------------------------------------------------------------------------------
	//
	// Reprise de donnÈes pour les blogs, 
	//
	// ---------------------------------------------------------------------------------
	case 'REPRISE_DATA' :
		if (phpCAS::isAuthenticated()) {
